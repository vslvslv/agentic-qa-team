...
<!-- See also: qa-web/references/playwright-patterns.md — verified against v1.60; iteration 26 (2026-05-12) adds: toHaveAccessibleName/toHaveRole/toHaveAccessibleDescription assertions (v1.44+), toBeChecked({indeterminate}) (v1.50+), --test-list/--test-list-invert CLI (v1.56+), context.clearCookies() filtering (v1.43+), context.isClosed() guard (v1.59+), step.skip()/step.attach() inside test.step body (v1.51+), consoleMessage.timestamp() (v1.59+), Speedboard HTML reporter tab (v1.57+), popup first request context.route gotcha (#28), addLocatorHandler mouse-position gotcha (#29), toMatchAriaSnapshot placeholder change gotcha (#30); 30 community gotchas total; 6500+ lines -->
...
<!-- See also: qa-perf/references/k6-patterns.md — verified against k6 v1.7.1; v2.0.0 final (2026-05-11) fully documented including: externally-controlled executor removed, CLI commands removed, browser_web_vital_fid → browser_web_vital_inp, options.ext.loadimpact → options.cloud, --no-summary → --summary-mode=disabled, --summary-mode=legacy removed, HTTP API server disabled by default (use --address/K6_ADDRESS), cloud secrets auto-injected in --local-execution (opt-out: --no-cloud-secrets), k6 cloud project list command added; HTTP/2 section added; Streams API (k6/experimental/streams) section added; 37 community gotchas including gotchas 34 (bidi stream.end() in finally), 35 (BrowserContext not shareable across VUs), 36 (browser Locator auto-retries in v2.0 — flaky selectors now silent), 37 (http.get() extra-arg warning in v2.0); new patterns: gRPC client-side streaming, bidirectional streaming, gRPC reflection (connect({reflect:true})), Client.healthCheck() health protocol, gRPC maxReceiveSize/maxSendSize, BrowserContext auth cookie injection (storageState equivalent), browser device emulation (devices object + mobile profiles + side-by-side desktop/mobile Web Vitals), k6 new --template CLI scaffolding, k6 x docs/explore commands, K6_PROVISION_HOST_VERSION env var, dashboard built into v2.0 core (no xk6-dashboard extension needed) -->
...
**See also:** `qa-web/references/cypress-patterns.md` (TypeScript/JavaScript) — 113+ patterns including Shadow DOM, GraphQL intercept, CDP throttling, cy.all(), WebKit testing, Smart Orchestration, Cypress Cloud MCP integration, cy.press() Tab-order tests, cy.env() async secure vars, Cypress.expose() public config, Cypress 15 breaking changes (exitCode/Vite ESM/Firefox CDP/Node.js 22), Angular 21 zoneless CT, cy.readFile() query promotion, TypeScript 6 + Vite 8 setup, Svelte 5 CT (runes/callback props), React 19 CT, GitHub Actions v7 + Node.js 24 CI patterns, cy.prompt() AI test authoring (Cy 15.13+), Cypress.stop() fail-fast (Cy 14.2+), --posix-exit-codes/--pass-with-no-tests CLI flags (Cy 15.4/15.11), experimentalFastVisibility (Cy 15.8), Firefox WebDriver BiDi CDP guard pattern (Cy 14.1+), justInTimeCompile webpack CT default, experimentalRunAllSpecs for component testing (Cy 15.9+), React SSR hydration bootstrap script (data-cy-bootstrap, Cy 15.11+), Cypress.ElementSelector.defaults() selectorPriority config (Cy 15+), and 89+ [community] gotchas (including cy.intercept() delay >= 2^31 validation, <base target> iframe navigation break, cypress open memory leak, allowCypressEnv:false third-party plugin pitfall)
