...
> Key patterns: POM + fixture injection · storageState auth (single/multi-role/API-based/worker-scoped) · IndexedDB in storageState (v1.51+) · OAuth + MFA + magic link auth · locator rank · web-first assertions · soft assertions · network mocking + HAR recording + network-cache · test sharding · visual regression · `expect.poll`/`toPass` · accessibility with axe-core · aria snapshots (`toMatchAriaSnapshot` + `locator.ariaSnapshot()`) · test annotations/tagging · global setup via project deps · custom reporters · multi-environment projects · custom matchers · keyboard/focus testing · browser storage manipulation · performance timing · mobile emulation · clock mocking · geolocation · test attachments · debug workflow · TypeScript config + ESLint · typed POM factory · WebSocket testing + `routeWebSocket` · pure API test suites · `addLocatorHandler` overlay dismissal · post-facto inspection APIs (`consoleMessages`/`pageErrors`/`requests`) · `locator.normalize()` · `locator.describe()` · Screencast API + `showActions()` · Docker deployment · auto-fixture suite scaling · `browser.bind()` multi-client · breaking changes reference (v1.45–v1.59)
> Breaking changes: `page.accessibility` removed (v1.57) → use `toMatchAriaSnapshot` / axe-core · Chrome for Testing replaces Chromium builds (v1.57) · React 16/17 CT support removed (v1.57)
