...
<!-- lang: JavaScript | topic: test-pyramid | iteration: 2 | score: 100/100 | date: 2026-04-30 -->
...
| React/Node frontend | Use Testing Trophy weighting — lean on integration test cases over unit test cases for UI logic |
...
- **Unit** — tests a single function/module in isolation; dependencies stubbed or mocked; runs in < 10 ms per test case.
...
```javascript
// Unit test case — isolated, no I/O (Jest + JavaScript)
...
describe('calculateDiscount', () => {
  ...
  it('throws for unknown membership tier', () => {
    expect(() => calculateDiscount({ total: 150, membershipTier: 'vip' }))
...
Integration test cases should exercise the real storage layer — not a mocked repository — to catch ORM quirks, constraint violations, and query N+1 problems that unit test cases cannot see.

```javascript
// Integration test case — real Postgres via testcontainers (Jest + JavaScript)
...
E2e test cases are expensive — reserve them for the paths that, if broken, would immediately stop revenue or access.

```javascript
// E2e test case — Playwright + JavaScript
...
Kent C. Dodds observed that for UI-heavy React applications, the classic pyramid under-weights integration test cases. In his *Testing Trophy* model the largest layer is **integration** — components rendered against their real hooks and context, with mocked network only at the boundary.

The four layers from bottom to top:
1. **Static analysis** (ESLint, TypeScript) — free confidence, no runtime needed.
...
```javascript
// Integration test case (Testing Trophy) — React Testing Library + MSW v2 + userEvent v14
...
When your service is a plain Express or Fastify app, `supertest` gives you a genuine integration test case against the running HTTP layer without needing a browser. This tests middleware stacks, request parsing, and response serialisation — all layers that pure unit test cases skip.
...
Without automated enforcement, pyramid shape drifts over time. The simplest guard is a Jest/Vitest output parser that counts test cases by directory convention and fails CI when the shape inverts.
...
Going too far in the other direction: mocking every dependency so that no real I/O ever runs. The result is test cases that pass even when the ORM query is wrong, the SQL constraint is missing, or the HTTP client constructs the wrong URL. [community] The unit test suite becomes a specification of the mocks, not the test object.

### Testing Implementation Details

Writing assertions on private methods, internal state, or component instance variables. These test cases break on every refactor even when the behaviour is unchanged. [community] Signal: you are asserting on `wrapper.state().isLoading` (Enzyme-style) instead of `screen.getByRole('status')` (RTL-style).
...
Enforcing "70/20/10" as a hard CI gate is counterproductive. [community] A CLI tool that does pure data transformation may legitimately have 95% unit test cases. Use ratios to diagnose imbalance, not as compliance checkboxes.
...
1. **Testcontainers start-up time blows integration test budgets** [community] — Teams discover that spinning up Postgres + Redis per test file takes 2–3 minutes. Fix: use a shared container per test suite (`beforeAll` not `beforeEach`), or point at a pre-provisioned service in CI.

2. **MSW and Playwright interact badly** [community] — Using MSW in integration test cases and Playwright in e2e against the same codebase requires keeping handler definitions in sync. When the real API changes, MSW handlers silently diverge. Fix: generate MSW handlers from OpenAPI schemas (e.g. `orval` or `msw-auto-mock`) so drift is a build-time error.

3. **Jest module mocking poisons adjacent test cases** [community] — `jest.mock()` at the module level with `jest.resetModules()` omitted causes test order-dependent failures. In a monorepo with thousands of test cases, this is the single most common source of "passes locally, fails in CI" defects. Fix: prefer dependency injection over `jest.mock()` for stateful modules.

4. **100% unit coverage hides zero integration confidence** [community] — A service can have 100% line coverage on its unit test level and completely fail at runtime because every dependency is mocked. Teams misread green coverage as "ship it". Fix: require a non-zero integration test count in CI merge gates, separately from coverage thresholds.
...
| Pure functions / algorithmic library | Near-100% unit test cases is correct — almost no integration surface |
...
- **Playwright configuration** (parallelism, retries, sharding) requires senior engineering time to tune; getting it wrong produces more flakiness than no e2e test cases at all.
